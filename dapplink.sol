...
contract DapplinkInfrastructureDeployment {
    ...
    uint public deployment_apk_fee;
    uint public deployment_apk_bonus;

    uint public deployment_dpwa_fee;
    uint public deployment_dpwa_bonus;

    uint public deployment_nft_fee;
    uint public deployment_nft_bonus;

    address[] public WWWSites;
    address[] public APKSites;
    address[] public DPWASites;
...
contract DapplinkInfrastructureListing {
    ...
    uint public request_listing_apk_fee;
    mapping( address => bool ) public listingAPK;
    event ListingRequestAPK   ( address _contract );
    event DelistingRequestAPK ( address _contract );

    uint public request_listing_dpwa_fee;
    mapping( address => bool ) public listingDPWA;
    event ListingRequestDPWA   ( address _contract );
    event DelistingRequestDPWA ( address _contract );
...
contract DapplinkInfrastructure is
    ...
    function deployAPK
        ...
        public returns ( address )
        {
            require
                (
                    balances[ msg.sender ] >= deployment_apk_fee,
                    "no enough tokens for deployment"
                );
            DapplinkSite site = deploy( msg.sender );
            balances[ msg.sender ] = safeSub(  balances[ msg.sender ], deployment_apk_fee    );
            balances[ msg.sender ] = safeAdd(  balances[ msg.sender ], deployment_apk_bonus  );
            APKSites.push(  address( site )  );
            listingAPK[ address(site) ] = false;
            emit DeployAPK
                    (
                            address( site ),
                            msg.sender
    ...
    function setDeploymentAPKFee
        (
            uint _fee
        )
        public onlyOwner returns ( bool success )
        {
            deployment_apk_fee = _fee;
            return true;
        }


    function setDeploymentAPKBonus
        (
            uint _bonus
        )
        public onlyOwner returns ( bool success )
        {
            deployment_apk_bonus = _bonus;
            return true;
        }


    function getNumberOfAPK
        ()
        public view returns ( uint count)
        {
            return APKSites.length;
        }
	...
	public
	{
            ...
            require
                    (
                        balances[ msg.sender ] >= request_listing_apk_fee,
                        "no enough tokens for listing request"
                    );
            emit ListingRequestAPK( _site );
        }
	...
	public
	{
            ...
            emit DelistingRequestAPK( _site );
        }
    ...
    function setRequestListingAPKFee
        (
            uint _fee
        )
        public onlyOwner
        {
            request_listing_apk_fee = _fee;
        }
    ...
    function deployDPWA
        ...
        public returns ( address )
        {
            require
                (
                    balances[ msg.sender ] >= deployment_dpwa_fee,
                    "no enough tokens for deployment"
                );
            DapplinkSite site = deploy( msg.sender );
            balances[ msg.sender ] = safeSub(  balances[ msg.sender ], deployment_dpwa_fee    );
            balances[ msg.sender ] = safeAdd(  balances[ msg.sender ], deployment_dpwa_bonus  );
            DPWASites.push(  address( site )  );
            listingDPWA[ address(site) ] = false;
            emit DeployDPWA
                    (
                            address( site ),
                            msg.sender
    ...
    function setDeploymentDPWAFee
        (
            uint _fee
        )
        public onlyOwner returns ( bool success )
        {
            deployment_dpwa_fee = _fee;
            return true;
        }


    function setDeploymentDPWABonus
        (
            uint _bonus
        )
        public onlyOwner returns ( bool success )
        {
            deployment_dpwa_bonus = _bonus;
            return true;
        }


    function getNumberOfDPWA
        ()
        public view returns ( uint count)
        {
            return DPWASites.length;
        }
    
    
    function getDPWAItem
        (
            uint _n     
        )
        public view returns ( address site )
	...
	public
	{
            ...
            require
                    (
                        balances[ msg.sender ] >= request_listing_dpwa_fee,
                        "no enough tokens for listing request"
                    );
            emit ListingRequestDPWA( _site );
        }
	...
	public
	{
            ...
            emit DelistingRequestDPWA( _site );
        }
    ...
    function setRequestListingDPWAFee
        (
            uint _fee
        )
        public onlyOwner
        {
            request_listing_dpwa_fee = _fee;
        }
    ...
    function setDeploymentNFTFee
        (
            uint _fee
        )
        public onlyOwner returns ( bool success )
        {
            deployment_nft_fee = _fee;
            return true;
        }


    function setDeploymentNFTBonus
        (
            uint _bonus
        )
        public onlyOwner returns ( bool success )
        {
            deployment_nft_bonus = _bonus;
            return true;
        }


    function getNumberOfNFT
        ()
        public view returns ( uint count)
        {
            return NFTSites.length;
        }
	...
	public
	{
            ...
            require
                    (
                        balances[ msg.sender ] >= request_listing_nft_fee,
                        "no enough tokens for listing request"
                    );
            emit ListingRequestNFT( _site );
        }
	...
	public
	{
            ...
            emit DelistingRequestNFT( _site );
        }
    ...
    function setRequestListingNFTFee
        (
            uint _fee
        )
        public onlyOwner
        {
            request_listing_nft_fee = _fee;
        }
